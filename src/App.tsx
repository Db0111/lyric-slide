...
export default function App() {
    ...
    slides.forEach((slideLines) => {
      ...
      if (showTitle && titleText) {
        ...
        switch (titlePosition) {
          case 'TL': x = 0.5; y = 0.3; align = 'left'; break;
          case 'TC': x = '50%'; y = 0.3; align = 'center'; break;
          case 'TR': x = 6.5; y = 0.3; align = 'right'; break;
          case 'BL': x = 0.5; y = 5.0; align = 'left'; break;
          case 'BC': x = '50%'; y = 5.0; align = 'center'; break;
          case 'BR': x = 6.5; y = 5.0; align = 'right'; break;
        }
  ...
  const PositionButton = ({ pos, label }: { pos: TitlePosition, label: string }) => (
    <button
    ...
    >
      <div className={`w-1.5 h-1.5 rounded-full bg-current ${
        pos.includes('L') ? 'mr-auto ml-1.5' : pos.includes('R') ? 'ml-auto mr-1.5' : 'mx-auto'
      } ${
  ...
  return (
    <div className="h-screen bg-slate-50 flex flex-col md:flex-row font-sans text-slate-900 overflow-hidden">
      ...
      <div className="w-full md:w-[485px] flex flex-col bg-white border-r border-slate-200 shadow-sm z-20">
          ...
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 flex items-center justify-center overflow-hidden rounded-xl">
              <img 
                ...
                onError={(e) => {
                  e.currentTarget.src = 'https://raw.githubusercontent.com/lucide-icons/lucide/main/icons/music.svg';
                  e.currentTarget.className = "w-8 h-8 text-indigo-600 opacity-50";
                }}
              />
            </div>
            <div className="flex flex-col">
              <h1 className="text-2xl font-medium tracking-tight leading-none text-slate-800">LyricSlide</h1>
              <span className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-1">가사를 PPT 슬라이드로</span>
            </div>
          </div>
          {lyrics && (
            <button 
        ...
        <div className="flex-1 p-6 flex flex-col min-h-0">
          <div className="mb-4 space-y-3">
            <label className="text-sm font-bold text-slate-500 uppercase tracking-widest">가사 입력</label>
      ...
      <div className="flex-1 flex flex-col min-h-0 bg-slate-100">
          ...
          <div className="max-w-4xl mx-auto w-full flex flex-col h-full overflow-hidden">
            <div className="flex items-center justify-between mb-4 shrink-0">
              <h2 className="text-sm font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
                <Monitor className="w-4 h-4" /> 슬라이드 미리보기 ({slides.length})
              </h2>
              {slides.length > 0 && (
                <div className="flex items-center gap-4">
            ...
            <div className="flex-1 relative min-h-0 w-full flex items-center justify-center bg-slate-200/30 rounded-3xl overflow-hidden p-4 md:p-8">
              {slides.length === 0 ? (
                <div className="w-full h-full flex flex-col items-center justify-center text-slate-400">
                  ...
                  <p className="font-bold text-lg text-center">가사를 입력하면<br/>미리보기가 표시됩니다.</p>
                </div>
              ) : (
                <div className="w-full h-full flex items-center justify-center">
                  <div className="relative w-full h-full flex items-center justify-center">
                    {/* 16:9 Aspect Ratio Container that fits within parent */}
                    <div className="relative w-full max-w-full max-h-full shadow-2xl overflow-hidden rounded-xl border border-slate-300" 
                          ...
                          {showTitle && titleText && (
                            <div 
                              className={`absolute p-6 w-1/3 z-10 ${
                                ...
                                titlePosition.includes('L') ? 'left-0 text-left' : 
                                titlePosition.includes('R') ? 'right-0 text-right' : 
                              ...
                              style={{ 
                                ...
                                fontWeight: 'bold',
                                fontFamily: FONTS.find(f => f.name === fontFamily)?.value || fontFamily
                              }}
                            >
                              {titleText}
                            </div>
        ...
        <div className={`bg-white transition-all duration-300 ease-in-out shadow-[0_-4px_30px_rgba(0,0,0,0.05)] flex flex-col ${isSettingsOpen ? 'flex-[2] min-h-[300px]' : 'h-14'}`}>
          <div 
          ...
          >
            <div className="flex items-center gap-2">
              <Settings className={`w-4 h-4 text-slate-400 transition-transform duration-500 ${isSettingsOpen ? 'rotate-90' : 'rotate-0'}`} />
              <span className="text-sm font-bold text-slate-500 uppercase tracking-widest">상세 설정</span>
            </div>
            <div className="flex items-center gap-4">
              {!isSettingsOpen && (
                <div className="hidden md:flex items-center gap-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                  <span className="flex items-center gap-1"><Palette className="w-3 h-3" /> 디자인</span>
                  <span className="flex items-center gap-1"><Type className="w-3 h-3" /> 제목</span>
                  <span className="flex items-center gap-1"><Layout className="w-3 h-3" /> 레이아웃</span>
                </div>
              )}
              {isSettingsOpen ? <ChevronDown className="w-5 h-5 text-slate-400" /> : <ChevronUp className="w-5 h-5 text-slate-400" />}
            </div>
          </div>

          <div className={`p-8 overflow-y-auto transition-opacity duration-300 ${isSettingsOpen ? 'opacity-100' : 'opacity-0 pointer-events-none h-0 p-0'}`}>
              ...
              <div className="space-y-8">
                <section>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <span className="text-sm text-slate-500 font-bold">배경색</span>
                      <div className="flex items-center gap-3 bg-slate-50 p-2 rounded-xl border border-slate-100">
                        ...
                        />
                        <span className="text-sm font-mono font-bold uppercase text-slate-600">{bgColor}</span>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <span className="text-sm text-slate-500 font-bold">글자색</span>
                      <div className="flex items-center gap-3 bg-slate-50 p-2 rounded-xl border border-slate-100">
                        <input 
                          type="color" 
                          value={textColor} 
                          ...
                          className="w-8 h-8 rounded-lg cursor-pointer border-none p-0 bg-transparent"
                        />
                        <span className="text-sm font-mono font-bold uppercase text-slate-600">{textColor}</span>
                      </div>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <span className="text-sm text-slate-500 font-bold">폰트 설정</span>
                ...
                <section className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${showTitle ? 'translate-x-6' : 'translate-x-1'}`} />
                    </button>
                  </div>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm font-bold text-slate-500">
                          <div className="flex items-center gap-2">
                            <span>제목 크기</span>
                            <span className="text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-md font-bold">추천: 18px</span>
                          </div>
                          <span className="text-indigo-600">{titleFontSize}px</span>
                        </div>
                        <input 
                  ...
                  <div className="space-y-5">
                    ...
                    {splitMode === 'fixed' && (
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm font-bold text-slate-500">
                          ...
                          <span className="text-indigo-600">{linesPerSlide}</span>
                        </div>
                        <input 
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm font-bold text-slate-500">
                        <div className="flex items-center gap-2">
                          <span>가사 글자 크기</span>
                          <span className="text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-md font-bold">추천: 34~36px</span>
                        </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex flex-col items-center text-center">
                ...
                <h3 className="text-xl font-black text-slate-900 mb-2">가사를 비우시겠습니까?</h3>
                <p className="text-slate-500 font-medium mb-8">
                  입력하신 모든 가사가 삭제됩니다.<br/>이 작업은 되돌릴 수 없습니다.
                </p>
